...
function useSync<T>(key: string, init: T, userId: string | null): [T, (fn: T | ((p: T) => T)) => void] {
  ...
  useEffect(() => { try { localStorage.setItem(lsKey, JSON.stringify(val)); } catch {} }, [lsKey, val, key, userId]);
  return [val, setVal];
}
          ...
          <div className="mt-3">
            {HAS_SB ? (
              <span className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-emerald-100 text-emerald-700 text-xs font-bold">
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />Cloud Sync
              </span>
            ) : (
...
function Dashboard({ user, onLogout }: { user: AppUser; onLogout: () => void }) {
  ...
  const [examSubs, setExamSubs] = useSync<string[]>("examsubs", [], userId);
  ...
  const markRevised = useCallback((id: string) => {
    setTopics(p => p.map(t => { if (t.id !== id) return t; const nl = t.lvl + 1; return { ...t, lvl: nl, next: nxt(iso(), nl), revs: [...t.revs, { date: iso(), ok: true, lvl: nl }] }; }));
  }, []);
  const undoRev = useCallback((id: string) => {
    setTopics(p => p.map(t => {
      if (t.id !== id || !t.revs.length) return t;
      const rv = t.revs.slice(0, -1); const nl = Math.max(0, t.lvl - 1);
      const nr = rv.length ? nxt(rv[rv.length - 1].date, rv.length - 1) : t.date;
      return { ...t, lvl: nl, next: nr, revs: rv };
    }));
  }, []);
  const updateRevDate = useCallback((tid: string, revIdx: number, newDate: string) => {
    setTopics(p => p.map(t => {
      if (t.id !== tid) return t;
      const newRevs = t.revs.map((r, i) => i === revIdx ? { ...r, date: newDate } : r);
      return { ...t, revs: newRevs, next: nxt(newDate, t.lvl) };
    }));
    setRevEdit(null);
  }, []);
  const deleteRev = useCallback((tid: string, revIdx: number) => {
    setTopics(p => p.map(t => {
      if (t.id !== tid || !t.revs[revIdx]) return t;
      const newRevs = t.revs.filter((_, i) => i !== revIdx);
      const nl = newRevs.length;
      const nr = newRevs.length ? nxt(newRevs[newRevs.length - 1].date, nl - 1) : t.date;
      return { ...t, lvl: nl, revs: newRevs, next: nr };
    }));
  }, []);
  ...
  const runAI = useCallback(async (eid: string) => { const ex = exams.find(e => e.id === eid); if (!ex?.answer) return; setAiId(eid); try { const r = await (window as any).puter.ai.chat(`Evaluate: ${ex.answer}`, { model: "perplexity/sonar" }); setExams(p => p.map(e => e.id === eid ? { ...e, ai: typeof r === "string" ? r : r?.message?.content || "Error" } : e)); } catch { setExams(p => p.map(e => e.id === eid ? { ...e, ai: "AI error" } : e)); } finally { setAiId(null); } }, [exams]);
  ...
  // Simplified Exam Form with dynamic subjects
  const ExamForm = () => {
    ...
    return (
      <Modal onClose={() => { setModal(null); setEditM(null); }}>
        <form onSubmit={save} className="p-6 space-y-4">
          ...
          <textarea value={content} onChange={e => sCont(e.target.value)} rows={6} placeholder="Content (supports bold, italic, colors, lists)..." className="w-full px-4 py-3 rounded-xl border outline-none resize-none" />
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-indigo-50/30">
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { if (tab === "topics") { setEditT(null); setModal("topic"); } else if (tab === "exams") { setEditE(null); setModal("exam"); } else { setEditM(null); setModal("memory"); } }} className="px-4 py-2.5 rounded-xl text-sm font-semibold text-white bg-indigo-600"><IPlus sz={16} /></button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex gap-2 bg-white rounded-2xl p-1.5 border w-fit">
          {[["topics", "📚", "Topics", topics.length], ["exams", "📝", "Exams", exams.length], ["memory", "🧠", "Memory", memory.length]].map(([k, e, l, n]) => (
            <button key={k} onClick={() => { setTab(k as Tab); setCf("all"); }} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold ${tab === k ? (k === "topics" ? "bg-indigo-100 text-indigo-700" : k === "exams" ? "bg-amber-100 text-amber-700" : "bg-teal-100 text-teal-700") : "text-gray-500 hover:bg-gray-50"}`}>{e} {l} ({n})</button>
          ))}
        </div>
        ...
        {tab === "topics" && topics.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            {[["Total", topics.length, "📚", "blue"], ["Overdue", over, "⚠️", "red"], ["Due Today", due, "🔔", "amber"], ["Scheduled", sched, "📅", "violet"], ["Upcoming", topics.length - over - due - sched, "📈", "emerald"]].map(([l, v, e, c]) => (
              <div key={l} className="bg-white rounded-2xl border p-4 shadow-sm">
                <div className="flex items-center gap-3"><div className={`bg-${c}-50 p-2.5 rounded-xl`}><span className="text-lg">{e}</span></div><div><p className="text-2xl font-bold">{v}</p><p className="text-xs text-gray-500">{l}</p></div></div>
              </div>
            ))}
          </div>
        )}
        ...
        {curItems.length > 0 && (
          <div className="space-y-3">
            <div className="relative"><ISearch sz={18} cls="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" /><input value={q} onChange={e => setQ(e.target.value)} placeholder={`Search ${tab}...`} className="w-full pl-12 pr-4 py-3 rounded-2xl bg-white border outline-none" /></div>
            <div className="flex gap-2 overflow-x-auto pb-1">
              <button onClick={() => setCf("all")} className={`px-4 py-2 rounded-xl text-xs font-semibold whitespace-nowrap border-2 ${cf === "all" ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-600 border-gray-200"}`}>All</button>
              {curCats.map(c => { const n = tab === "topics" ? topics.filter(t => t.cat === c).length : tab === "exams" ? exams.filter(e => e.cat === c).length : memory.filter(m => m.cat === c).length; return <button key={c} onClick={() => setCf(cf === c ? "all" : c)} className={`px-4 py-2 rounded-xl text-xs font-semibold whitespace-nowrap border-2 ${cf === c ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-600 border-gray-200"}`}>{c}{n > 0 && <span className="ml-1.5 opacity-70">({n})</span>}</button>; })}
            </div>
          </div>
        )}

        {/* TOPICS LIST */}
        {view === "list" && tab === "topics" && (topics.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border"><div className="w-20 h-20 bg-indigo-50 rounded-3xl mx-auto mb-6 flex items-center justify-center"><span className="text-4xl">📚</span></div><h3 className="text-xl font-bold mb-2">No topics yet</h3><p className="text-gray-500 mb-8">Start tracking your revision</p><button onClick={() => setModal("topic")} className="px-6 py-3 rounded-xl text-white bg-indigo-600 font-semibold">+ Add Topic</button></div>
